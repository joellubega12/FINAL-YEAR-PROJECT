...
def voters(request):
    voters = Voter.objects.all()
    userForm = CustomUserForm(request.POST or None, is_admin=True)
...
def updateVoter(request):
    ...
    try:
        ...
        user = CustomUserForm(request.POST or None, instance=instance.admin, is_admin=True)
...
def resetVote(request):
    ...
    Voter.objects.all().update(voted=False, verified=True, otp=None)
